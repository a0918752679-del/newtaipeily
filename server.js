...
app.get('/healthz', (req,res)=>res.json({ok:true, service:'newtaipei-noise-control-system-v18-security', hasAdminPassword:!!ADMIN_PASSWORD, hasSessionSecret:!!SESSION_SECRET}));
app.get('/api/line/test', (req,res)=>res.json({ok:true, service:'v18-security', hasToken:!!LINE_TOKEN, hasSecret:!!LINE_SECRET, hasAdminPassword:!!ADMIN_PASSWORD, hasSessionSecret:!!SESSION_SECRET}));
...
function richMenuSpec(){
  return { size:{width:2500,height:1686}, selected:true, name:'新北噪音車V17正式版圖文選單', chatBarText:'管理選單', areas:[
    {bounds:{x:0,y:310,width:625,height:688}, action:{type:'uri', uri:DASHBOARD_URL}},
    {bounds:{x:625,y:310,width:625,height:688}, action:{type:'uri', uri:FIELD_REPORT_URL}},
    {bounds:{x:1250,y:310,width:625,height:688}, action:{type:'message', text:'車號追蹤'}},
    {bounds:{x:1875,y:310,width:625,height:688}, action:{type:'message', text:'KPI報表'}},
    {bounds:{x:0,y:998,width:625,height:688}, action:{type:'message', text:'統計選單'}},
    {bounds:{x:625,y:998,width:625,height:688}, action:{type:'message', text:'法規中心'}},
    {bounds:{x:1250,y:998,width:625,height:688}, action:{type:'message', text:'設備管理'}},
    {bounds:{x:1875,y:998,width:625,height:688}, action:{type:'message', text:'管理功能'}}
  ]};
}
...
body{margin:0;font-family:Arial,'Noto Sans TC','Microsoft JhengHei',sans-serif;background:#eef6ff;color:#06295c}.hero{background:linear-gradient(120deg,#003b8e,#00a6ff);color:white;padding:30px 36px;border-radius:0 0 28px 28px;box-shadow:0 12px 30px #b8d7f0}.wrap{max-width:1160px;margin:28px auto;padding:0 20px}.grid{display:grid;grid-template-columns:repeat(auto-fit,minmax(260px,1fr));gap:18px}.card{background:white;border:1px solid #c9dff3;border-radius:20px;padding:22px;box-shadow:0 8px 20px #cfe2f2}.btn{display:inline-block;border:0;border-radius:12px;background:#006ee6;color:white;padding:13px 18px;font-weight:700;cursor:pointer;text-decoration:none}.btn2{background:#00a77a}.btn3{background:#4b5a6a}.muted{color:#62748a}.code{background:#e9f2fb;border-radius:12px;padding:14px;overflow:auto}input[type=file]{padding:10px;background:#f6fbff;border:1px solid #bfd7ed;border-radius:10px}</style></head><body>${body}</body></html>`}
...
app.listen(PORT, ()=> console.log(`New Taipei V17 production running on :${PORT}`));
